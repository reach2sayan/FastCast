...
BOOST_AUTO_TEST_CASE(Simple_Success_Ptr) {
  ...
  auto *fp = fast_dynamic_cast<SimpleB *>(a);
...
BOOST_AUTO_TEST_CASE(Simple_Failure_PtrNull) {
  ...
  BOOST_CHECK(fast_dynamic_cast<SimpleB *>(a) == nullptr);
  delete a;
}
...
BOOST_AUTO_TEST_CASE(Simple_Failure_BadCastRef) {
  ...
  try {
    (void)fast_dynamic_cast<SimpleB &>(a);
    BOOST_FAIL("Expected std::bad_cast");
  } catch (const std::bad_cast &) {
    BOOST_CHECK(true);
  }
...
BOOST_AUTO_TEST_CASE(Simple_ConstCorrectness) {
  ...
  auto &fr = fast_dynamic_cast<const SimpleB &>(a);
...
BOOST_AUTO_TEST_CASE(Complex_Success_Ref) {
  ...
  auto &fr = fast_dynamic_cast<ComplexE &>(x);
...
BOOST_AUTO_TEST_CASE(Complex_Success_CrossCast) {
  ...
  auto *fz = fast_dynamic_cast<ComplexB *>(x);
...
BOOST_AUTO_TEST_CASE(Complex_Failure_NullPtr) {
  ...
  BOOST_CHECK(fast_dynamic_cast<ComplexF *>(x) == nullptr);
}
...
BOOST_AUTO_TEST_CASE(Complex_Failure_Unrelated) {
  ...
  try {
    (void)fast_dynamic_cast<ComplexF &>(reinterpret_cast<ComplexA &>(z1));
    BOOST_FAIL("Expected std::bad_cast");
  } catch (const std::bad_cast &) {
    BOOST_CHECK(true);
  }
...
BOOST_AUTO_TEST_CASE(SameType) {
  Derived d;
  Derived *dp = &d;
  auto r1 = fastcast::fast_dynamic_cast<Derived *>(dp);
  BOOST_CHECK(r1 == dp);
}
...
BOOST_AUTO_TEST_CASE(DerivedToBase_StaticCastPath) {
  Derived d;
  Derived *dp = &d;
  Base *bp = fastcast::fast_dynamic_cast<Base *>(dp);
...
BOOST_AUTO_TEST_CASE(BaseToDerived_RuntimePath) {
  Derived d;
  Base *bp = &d;
  auto dp = fastcast::fast_dynamic_cast<Derived *>(bp);
...
BOOST_AUTO_TEST_CASE(MultipleInheritance_Failure) {
  Multi m;
  Base *bp = &m;
  auto ap = fastcast::fast_dynamic_cast<AnotherBase *>(bp);
...
BOOST_AUTO_TEST_CASE(FailureCaching) {
  ...
  auto dp2 = fastcast::fast_dynamic_cast<Derived *>(bp);
